<template>
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <!--表单组件-->
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="2000px">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column prop="idCard" label="idCard"  width="150"/>
        <el-table-column prop="contract" label="contract" width="150" />
        <el-table-column prop="code" label="code" width="2000" />
        <el-table-column prop="name" label="name" />
        <el-table-column prop="date" label="date" />
        <el-table-column prop="money" label="money" />
        <el-table-column prop="term" label="term" />
        <el-table-column prop="company" label="company" width="500"/>
        <el-table-column prop="car" label="car" width="500"/>
        ...
        <el-table-column prop="notes" label="notes"  width="1000"/>
        <el-table-column prop="address" label="address" />

        <el-table-column prop="dueDate" label="dueDate" />
        <el-table-column v-permission="['admin','capitalHistory:edit','capitalHistory:del']" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
<script>
import crudCapitalHistory from '@/api/capitalHistory'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '历史中登数据', url: 'api/capitalHistory', idField: 'id', sort: 'id,desc', crudMethod: { ...crudCapitalHistory }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'capitalHistory:add'],
        edit: ['admin', 'capitalHistory:edit'],
        del: ['admin', 'capitalHistory:del']
      },
      rules: {
      }    }
  },
